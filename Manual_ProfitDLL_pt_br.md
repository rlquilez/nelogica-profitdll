...
fabricante: "Nelogica"
gerado_por: "conversão estruturada do PDF oficial"
---
...
- **As assinaturas Delphi são a referência canônica.** Toda função, callback e
  estrutura é declarada em Delphi/Pascal. Para C#, C++ ou Python, traduza os
  tipos usando a seção *Mapeamento de tipos por linguagem*.
- **Cada função e cada callback tem um heading `####` próprio** com o nome exato
  entre crases — use busca exata pelo nome da API para localizar sua seção.
- **Convenção de chamada:** `stdcall` em 32 e 64 bits, sem exceção.
- **Prefira sempre a API moderna.** Consulte *APIs deprecadas → substitutas*
  antes de usar qualquer função — o manual marca 29 APIs como obsoletas.
- **Estruturas `TConnector*` têm um campo `Version : Byte` que precisa ser
  preenchido** antes de cada chamada; versão não suportada devolve
  `NL_VERSION_NOT_SUPPORTED`.
- **Estruturas `*Out` exigem duas chamadas:** a primeira preenche os campos
  `*Length`, você aloca os buffers, e a segunda devolve o conteúdo.
- **Retornos são códigos `NResult`:** `NL_OK` é `0` e os erros são negativos.
  As funções de envio de ordem devolvem um ID local positivo em caso de
  sucesso — teste `< 0`, não `!= 0`.

### Regra crítica de callbacks

Todos os callbacks são executados na thread interna `ConnectorThread`, e todos
compartilham uma única fila de mensagens. Processamento lento dentro de um
callback atrasa a entrega de todos os demais.
...
> `GetTheoreticalValues` — são a exceção: elas existem para ser chamadas de
> dentro do callback que as acompanha.
...
| `SetHealthCallback` | 4.0.0.41 | Registra callback de saúde do *watchdog* interno |
| `TSystemHealthCallback` | 4.0.0.41 | Tipo do callback de saúde |
| `TSystemHealthState` | 4.0.0.41 | `shsResponsive` (0) / `shsFrozen` (1) |

### Constantes novas

| Constante | Valor | Versão | Significado |
|---|---|---|---|
| `MARKET_PERFORMANCE_WARNING` | 5 | 4.0.0.39 | Feed OK, entrega local de callbacks degradada |
| `MARKET_PARTIAL_CONNECTED` | 6 | 4.0.0.39 | Market data parcialmente conectado |
| `NL_HISTORY_PERIOD_LIMIT` | `0x8000002E` | 4.0.0.41 | Período de histórico acima do limite (30 dias) |
| `nTradeType` 14–18 | BBT, RFQ, MPT, TAC, TAA | 4.0.0.41 | Novos tipos de negócio |
| `nTradeType` 33–35 | Update, Mid, Off Exchange | 4.0.0.41 | Novos tipos de negócio |

### Mudança de ABI (atenção)

| Estrutura/callback | Mudança | Versão |
|---|---|---|
| `TAssetListInfoCallback` e `TAssetListInfoCallbackV2` | `nMinOrderQtd`, `nMaxOrderQtd` e `nLote` passaram de `Integer` para `Int64` | 4.0.0.38 |

Aplicações que registram essas callbacks **precisam** atualizar as assinaturas;
caso contrário a pilha é corrompida na chamada.

### Correções relevantes da DLL

| Correção | Versão |
|---|---|
| `SetTradeCallbackV2` parava de receber trades | 4.0.0.38 |
| Access Violation em `PopulateOrderOutV0` em picos de volume | 4.0.0.39 |
...
## APIs deprecadas → substitutas

Índice consolidado de todas as APIs que o próprio manual marca como obsoletas ou depreciadas. Em código novo, use sempre a coluna *Substituir por*.

| API deprecada | Substituir por |
|---|---|
| `SubscribePriceBook` | `SubscribePriceDepth` |
| `UnsubscribePriceBook` | `UnsubscribePriceDepth` |
| `GetAgentNameByID` | `GetAgentName` junto a `GetAgentNameLength` |
| `GetAgentShortNameByID` | `GetAgentName` junto a `GetAgentNameLength` |
| `SetHistoryCallbackV2` | `SetOrderHistoryCallback` |
...
| `SetHistoryCallback` | `SetOrderHistoryCallback` |
...
| `GetOrders` | `HasOrdersInInterval`, `EnumerateOrdersByInterval` e `EnumerateAllOrders` |
...
## Mapeamento de tipos por linguagem

O manual declara tudo em Delphi. Esta tabela resume a tradução usada pelos
exemplos oficiais que acompanham a DLL.
...
| `PWideChar` | `string` com `[MarshalAs(UnmanagedType.LPWStr)]` | `wchar_t*` | `c_wchar_p` |
| `Pointer` | `IntPtr` | `void*` | `c_void_p` / `c_size_t` |
| `record` | `struct` com `[StructLayout(LayoutKind.Sequential)]` | `struct` | `Structure` com `_fields_` |
| `^T` (ponteiro) | `ref T` / `out T` | `T*` | `POINTER(T)` / `byref(x)` |
| `procedure(...) stdcall` | `delegate` com `CallingConvention.StdCall` | `void(__stdcall*)(...)` | `WINFUNCTYPE(None, ...)` |

Referência de conversão citada pelo manual:
<http://www.netcoole.com/delphi2cs/datatype.htm>

> **Atenção (C#):** o coletor de lixo recolhe delegates assim que saem da pilha.
> Mantenha uma referência estática viva para cada callback registrado.

> **Atenção (Python):** `main.py` dos exemplos requer Python 3.10+ e, em 32
> bits, o manual recomenda a versão 3.6.2 por causa de um bug de callbacks com
> tipos maiores que 32 bits.

## Notas desta conversão
...
### Monitoramento de travamento das threads internas

Adicionado um mecanismo de *watchdog* que detecta quando as threads internas da DLL travam. O estado é agregado em `TSystemHealthState`: `shsResponsive` (0) quando Main e Calc estão respondendo e `shsFrozen` (1) quando qualquer uma das duas está travada.

### Logs de performance

Habilitada a geração de logs de performance (*sampling profiler*) na DLL de 64 bits. O monitor amostra periodicamente a pilha da thread principal e grava em arquivo, auxiliando o diagnóstico de lentidão e travamentos.

### Bug fixes

- Corrigida impossibilidade de conexão ao Market Data quando o processo hospedeiro da DLL;
- Corrigido erro ao assinar o livro de preços (`SubscribePriceDepth`) de um ativo inválido;
- Corrigida ausência de atualização nos callbacks de ordem: confirmações intermediárias de roteamento eram tratadas como definitivas, impedindo o consumo do buffer e a entrega das atualizações seguintes.

## 4.0.0.39

### Monitoramento do feed de market data

Adicionado mecanismo de *watchdog* que detecta quando o feed do servidor está OK, mas a entrega local de callbacks de market data está parada. Dois novos estados de market data passam a ser retornados em `TStateCallback` / `TNewStateCallback` (`nConnStateType = 2`): `MARKET_PERFORMANCE_WARNING` (5) e `MARKET_PARTIAL_CONNECTED` (6)

### Bug fixes

- Corrigido Access Violation em `PopulateOrderOutV0` durante picos de volume.
- Documentação: esclarecida a distinção entre funções de requisição e funções acessórias no uso de callbacks.

## 4.0.0.38

### Callbacks modificadas
...
- Ajustada requisição de histórico de ordens perdendo ordens passadas.
- Ajustado `UnsubscribePriceDepth` não enviando o pedido ao servidor.
- Ajustado `NewDailyCallback` não disparando.

## 4.0.0.36

### Bug fixes

- Adicionada flag `OB_FIRST_PACKET` nas callbacks de OfferBook.
- `GetAgentName` agora retorna corretamente o tamanho da string copiada.
- Exemplo de uso em C# refeito.

## 4.0.0.35

### Bug fixes

- Otimização no recebimento de contas de roteamento.
- Corrigida exception na notificação de download de trades.
...
- Ajustes no GetOrders do exemplo C#.
...
Para melhor gerenciar preços teóricos durante o leilão, foi criado um novo mecanismo para obter os dados do livro de preços. Ao usar a nova função `SubscribePriceDepth` e assinar a nova callback `SetPriceDepthCallback`, os dados do livro podem ser buscados pela função `GetPriceGroup`, que já traz uma entrada do livro pronta, após todos os cálculos.
...
- Arrumada a callback `SetTheoreticalPriceCallback` que não estava disparando.
...
| NL_SERIE_NO_MORE_HISTORY | 0x80000018 | -2147483624 | Não tem mais dados disponiveis para a serie |
...
| NL_HISTORY_PERIOD_LIMIT | 0x8000002E | -2147483602 | Período de histórico solicitado excede o limite permitido (data inicial com mais de 30 dias) |
...
| Nome | Tipo | Descrição |
|---|---|---|
| var nState | Integer | Recebe o estado agregado de saúde da DLL (`TSystemHealthState`) |

Consulta *pull* do estado de saúde interno da DLL. Retorna `shsResponsive` (0) quando as threads Main e Calc estão respondendo e `shsFrozen` (1) quando qualquer uma delas está travada. A função lê apenas o estado do *watchdog*, sem depender das threads monitoradas, portanto responde imediatamente mesmo com a Main ou o Calc congelados. Retorna `NL_NOT_INITIALIZED` caso a DLL ainda não tenha sido inicializada.
...
Utilizado para definar a função de callback do tipo `TStateCallback`. Sobrepõe a callback definida pelo `DLLInitializeLogin` ou `DLLInitializeMarketLogin`.

#### `SetHealthCallback`

Utilizado para definir a função de callback do tipo `TSystemHealthCallback`, que notifica sempre que o estado de saúde interno da DLL muda entre responsivo (`shsResponsive`) e travado (`shsFrozen`). Complementa a consulta *pull* `GetHealthStatus`.
...
Utilizado para definar a função de callback do tipo `THistoryTradeCallback`. Sobrepõe a callback definida pelo `DLLInitializeLogin` ou `DLLInitializeMarketLogin`.

#### `SetDailyCallback`

Utilizado para definar a função de callback do tipo `TDailyCallback`. Sobrepõe a callback definida pelo `DLLInitializeLogin` ou `DLLInitializeMarketLogin`.

#### `SetSerieProgressCallback`

Utilizado para definar a função de callback do tipo `TProgressCallback`. Sobrepõe a callback definida pelo `DLLInitializeLogin` ou `DLLInitializeMarketLogin`.

#### `SetOfferBookCallback`

Utilizado para definar a função de callback do tipo `TOfferBookCallback`. Sobrepõe a callback definida pelo `DLLInitializeLogin` ou `DLLInitializeMarketLogin`.
...
Utilizado para definar a função de callback do tipo `TConnectorAssetPositionListCallback`. Disparado quando a uma alteração nas posições da conta. Envia o `TConnectorAssetIdentifier` que foi alterado, ou caso a alteração tenha sido na lista completa, um `TConnectorAssetIdentifier` com o campo Ticker em -1. Sobrepõe a callback do tipo `TAccountCallback`.

#### `SetAccountCallback`

Utilizado para definar a função de callback do tipo `TAccountCallback`. Sobrepõe a callback definida pelo `DLLInitializeLogin`.
...
Utilizado para definiar uma função de callback do tipo `TConnectorPriceDepthCallback`. Disparado para cada atualização do livro de preço. Quando mais uma notificação ocorrerá em rápida sucessão, é disparado uma notificação com `Prepare` antes, e uma com `Flush` de todas as itermediárias.
...
Envia uma ordem de modificação. Aceita como parâmetro um ponteiro para uma estratura do tipo `TConnectorChangeOrder`. Aceita tanto contas, como subcontas. Estado da ordem pode ser acompanhado pela callback definida em `SetOrderCallback`.
...
|---|---|---|
| a_AccountID | PConnectorAccountIdentifier | Identificador da conta/subconta |
| a_AssetVersion | Byte | Versão do identificador do ativo para retornar em `a_Callback` |
| a_Param | LPARAM | Parâmetro definido pelo implementador, retornado em `a_Callback` |
...
Essa função traduz um trade recebido pelas callbacks do tipo `TConnectorTradeCallback`, e `TConnectorQuoteCallback` para as séries de 1-trade.
...
Corresponde ao callback que informa o estado de saúde interno da DLL, disparado sempre que o estado agregado das threads Main e Calc muda. O parâmetro `nState` recebe um valor de `TSystemHealthState`:
...
Esse callback avisa a condição via `nState = 1` e, ao normalizar, dispara novamente com `nState = 0`. A detecção é feita por um *watchdog* em thread própria, independente de qualquer loop de mensagens, e o mesmo estado pode ser consultado sob demanda pela função `GetHealthStatus`.
...
| nMinOrderQtd | Int64 | Mínima quantidade de ordens permitidas |
